...
def detect(image):
    ...
    #  Sort contours
    sorted_contours = sorted(contours, key=lambda ctr: openCV.boundingRect(ctr)[0])
    # # Removing the biggest contour which is the image box
    # del sorted_contours[0]
    # # Drawing contours
    # openCV.drawContours(image, sorted_contours, -1, (255, 0, 255), 3)
    # # Saving image with contours
    # openCV.imwrite("contour.png", image)
    # # openCV.imshow("Contours", image)
    # # openCV.waitKey(0)
    # # openCV.destroyAllWindows()
...
def load_puzzle(path=None):
    ...
    assert input_image is not None, "Please supply correct path to image!"
    return input_image
