...
PATCH_DIMENSIONS = -1
# The allowable piece on y and x axis, there are 2 possibilities in the case of a rectangle
# It will be list with 2 tuples
# First tuple will hold the (y, x) puzzle frame size
# Second tuple will hold the (x, y) puzzle frame size
# *HINT* All this is done because we can rotate stuff *HINT*
ROTATED_DIMENSIONS = None
...
def get_assembly_rotation(piece_a, piece_b, side_a, side_b):
    """
    ...
    mgc_specific_relation = None
    piece_swap = False
    x = Chunk.Chunk.global_side_location
    if Chunk.Chunk.global_side_location[piece_a] != comp_list:
        side_a = Chunk.Chunk.global_side_location[piece_a][side_a]

    if Chunk.Chunk.global_side_location[piece_b] != comp_list:
        side_b = Chunk.Chunk.global_side_location[piece_b][side_b]
