...
class Slicer:
    ...
    def save(self, image_to_write, transformed_images, slices, coordinate, rotations=None):
        ...
        file_locations = open(self.location + self.output_file_name + "_" + str(range_for_i * range_for_j) + "_" + str(
            self.angle) + ".txt", mode="w")
    ...
    # @staticmethod
    # def create_shuffled_image(image_to_write, sliced_images, slices):
    #     """
    ...
    #     # Shuffles the image slices inside the deque (Randomly)
    #     for i in range(len(sliced_images)):
    #         openCV.imshow(str(i), sliced_images[i])
    #         openCV.waitKey(0)
    #         openCV.destroyAllWindows()
    #     random.shuffle(sliced_images)
    #     # "*********************************"
    ...
    #
    #             # Acquires the height and width of the sliced image which is next in line
    #             height = Slicer.peek(sliced_images, 0)
    #             width = Slicer.peek(sliced_images, 1)
    #             """
    ...
    #             image_to_write[starting_point_y + ((i + 1) * slice_spacing):ending_point_y + ((i + 1) * slice_spacing), starting_point_x + ((j + 1) * slice_spacing): ending_point_x + ((j + 1) * slice_spacing)] = sliced_images.pop()
    #
...
def main():
    # Parameters that need to be given in order for the slicer to work
    to_where_positions = "../evaluation/location/cats/"
    to_where_puzzle = "../output/rotated"
    output_file_name = "big_cat"
    from_where = "../input/cat_big.jpg"
    rotation = 90
    patch_size = 640

    obj = Slicer(to_where_positions, to_where_puzzle, output_file_name, from_where, rotation,
                 patch_size)
    print("Loaded image from:", from_where)
    obj.patch_slice()
